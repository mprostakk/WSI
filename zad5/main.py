...
def read_model_from_file(filename: str = "model.txt") -> Network:
    with open(filename, 'rb') as file:
        network = pickle.load(file)
    return network


def save_model_to_file(network: Network, filename: str = "model.txt"):
...
def predict():
    ...
    network: Network = read_model_from_file()
    ...
    for x in test_data[50:]:
        pixels, number_label = x
        output = network.feed_forward(pixels)
        result: int = int(number_label)

        predicted_result = output.argmax()
        ...
        if predicted_result != result:
            ...
            pixels_image = np.reshape(pixels, (28, 28))
            ax_all[i].imshow(pixels_image, interpolation='nearest')
            ax_all[i].set_title(f"Truth: {result} / Predicted: {predicted_result}")
...
def main() -> None:
    ...
    learning_rate = 0.5
    network = Network([network_input_size, 25, 25, network_output_size],
                      learning_rate=learning_rate)

    train_data = prepare_data(all_images, all_labels)
    np.random.shuffle(train_data)
    # train_data = sorted(train_data, key=lambda data: data[1])
    ...
    for epoch in range(number_of_epochs):
        ...
        if epoch > 2:
            y = np.array([np.array(xi) for xi in test_confusion_matrix])
            print(123)
